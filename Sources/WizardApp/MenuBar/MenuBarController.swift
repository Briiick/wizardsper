...
/// observable box.
///
/// Wizard is an `LSUIElement` app: the status item is the only thing on screen
...
final class MenuBarController: NSObject {
    ...
    private func configureButton() {
        guard let button = statusItem.button else {
            ...
            Log.ui.error("NSStatusBar gave no button; Wizard has no menu bar item.")
            return
        }
        button.image = Self.icon(listening: false)
        button.image?.isTemplate = true
        button.imagePosition = .imageOnly
        button.toolTip = "Wizard"
        button.setAccessibilityLabel("Wizard")
        button.target = self
    ...
    private func refreshIcon() {
        ...
        button.image?.isTemplate = true
        button.toolTip = status.isListening ? "Wizard — listening" : "Wizard"
    }
    ...
    private func togglePopover() {
        ...
        // its buttons is spent activating Wizard instead.
        NSApp.activate()
    ...
    private func makeMenu() -> NSMenu {
        ...
        let quit = NSMenuItem(title: "Quit Wizard", action: #selector(quit), keyEquivalent: "q")
