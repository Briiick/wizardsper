...
func resolveModelDirectory(_ arguments: Arguments) -> URL {
    ...
    let installed = WizardsperPaths.modelDirectory(for: tier)
...
func transcribe(
    ...
    guard !samples.isEmpty else { throw WizardsperError.noAudioCaptured }
...
func commandTranscribe(_ arguments: Arguments) async throws {
    guard let path = arguments.positional.first else { fail("usage: wizardsper-cli transcribe <audio>") }
...
func commandSweep(_ arguments: Arguments) async throws {
    guard let path = arguments.positional.first else {
        fail("usage: wizardsper-cli sweep <audio> --reference \"...\"")
    }
    let audio = URL(fileURLWithPath: (path as NSString).expandingTildeInPath)
    let directory = resolveModelDirectory(arguments)
...
func commandHelp() {
    print(
        """
        wizardsper-cli — development harness for the Wizardsper recognition path

        USAGE
          wizardsper-cli probe        [--model DIR | --tier 560]
              Print each model's real input/output signatures and the framing arithmetic.

          wizardsper-cli transcribe <audio> [--model DIR | --tier 560] [--framing NAME]
