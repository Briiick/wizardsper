...
/// Wizardsper's settings, model management and history in one window.
///
...
struct DashboardView: View {
    ...
    private var settingsPane: some View {
        Form {
            ...
            Section("Startup") {
                Toggle("Launch Wizardsper at login", isOn: launchAtLogin)
    ...
    private func reconcileLaunchAtLogin() {
        ...
        loginItemError =
            ...
            ? "Approve Wizardsper under Login Items in System Settings." : nil
    }
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 18) {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    Text(WizardsperPaths.modelDirectory(for: settings.tier).path)
...
extension WizardsperKit.Settings {
    /// Previews must not scribble on the real defaults domain.
    fileprivate static func previewSettings() -> WizardsperKit.Settings {
        WizardsperKit.Settings(
            defaults: UserDefaults(suiteName: "com.bricken.wizardsper.preview") ?? .standard)
    }
}
