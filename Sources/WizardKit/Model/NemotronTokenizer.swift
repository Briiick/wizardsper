...
public struct NemotronTokenizer: Sendable {
    ...
    public init(contentsOf url: URL) throws {
        ...
        guard let raw = try JSONSerialization.jsonObject(with: data) as? [String: String] else {
            throw WizardError.tokenizerInvalid("expected a flat object of id → piece")
        }
        guard !raw.isEmpty else {
            throw WizardError.tokenizerInvalid("vocabulary is empty")
        }
        ...
        for (key, value) in raw {
            guard let id = Int(key), id >= 0 else {
                throw WizardError.tokenizerInvalid("token id “\(key)” is not a non-negative integer")
            }
