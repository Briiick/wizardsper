...
public enum MLArrayReader {
    ...
    public static func requireShape(
        ...
        guard actual.count == expected.count else {
            throw WizardError.modelShapeMismatch(name: name, expected: expected, actual: actual)
        }
    ...
    public static func output(
        ...
        guard let value = provider.featureValue(for: name)?.multiArrayValue else {
            throw WizardError.modelOutputMissing(name)
        }
        if let expecting {
    ...
    public static func argmax(_ array: MLMultiArray, named name: String) throws -> (
        ...
        guard count > 0 else { throw WizardError.modelOutputMissing(name) }
