...
public enum AudioFileLoader {
    ...
    public static func samples(at url: URL) throws -> [Float] {
        ...
        guard
            ...
            throw WizardsperError.audioEngineFailed("could not build the 16 kHz mono target format")
        }
        ...
        guard let input = AVAudioPCMBuffer(pcmFormat: source, frameCapacity: readChunk) else {
            throw WizardsperError.audioEngineFailed("could not allocate the read buffer")
        }
        ...
        guard let output = AVAudioPCMBuffer(pcmFormat: target, frameCapacity: outputCapacity)
            ...
            throw WizardsperError.audioEngineFailed("could not allocate the conversion buffer")
        }
        ...
        while true {
            ...
            if let failure = pull.failure {
                throw WizardsperError.audioEngineFailed("decode failed: \(failure.localizedDescription)")
            }
            ...
            case .inputRanDry:
                ...
                return samples
            case .error:
                throw WizardsperError.audioEngineFailed("conversion reported an error")
            @unknown default:
                return samples
            }
        }
    ...
    private static func readDirect(file: AVAudioFile, format: AVAudioFormat) throws -> [Float] {
        ...
        guard let buffer = AVAudioPCMBuffer(pcmFormat: format, frameCapacity: frames) else {
            throw WizardsperError.audioEngineFailed("could not allocate a \(frames)-frame buffer")
        }
