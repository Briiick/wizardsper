...
public struct ModelBundle: @unchecked Sendable {
    ...
    public static func load(from directory: URL) async throws -> ModelBundle {
        ...
        guard fileManager.fileExists(atPath: directory.path) else {
            throw WizardsperError.modelsMissing(directory.path)
        }

        let metadataURL = directory.appendingPathComponent("metadata.json")
        let tokenizerURL = directory.appendingPathComponent("tokenizer.json")
        guard fileManager.fileExists(atPath: metadataURL.path) else {
            throw WizardsperError.modelsMissing(metadataURL.path)
        }
        guard fileManager.fileExists(atPath: tokenizerURL.path) else {
            throw WizardsperError.modelsMissing(tokenizerURL.path)
        }
    ...
    private static func loadModel(
        ...
        guard FileManager.default.fileExists(atPath: url.path) else {
            throw WizardsperError.modelsMissing(url.path)
        }
        let configuration =
        ...
        do {
            ...
            throw WizardsperError.modelLoadFailed(relativePath, underlying: error.localizedDescription)
        }
    }
}
