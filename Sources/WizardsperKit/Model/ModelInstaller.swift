...
public actor ModelInstaller {
    ...
    ///
    /// A phase carries no tier: Wizardsper installs one tier at a time, and pairing
    ...
    public func isInstalled(_ tier: NemotronTier) -> Bool {
        Self.missingPiece(of: tier, in: WizardsperPaths.modelDirectory(for: tier)) == nil
    }
    ...
    private func performInstall(_ tier: NemotronTier) async throws -> URL {
        ...
        do {
            ...
            if let missing = Self.missingPiece(of: tier, in: directory) {
                throw WizardsperError.downloadFailed(
    ...
    public func importZip(at zipURL: URL, as tier: NemotronTier) async throws -> URL {
        let destination = WizardsperPaths.modelDirectory(for: tier)
        let manager = FileManager.default
        do {
            // Unpacking has no byte count to report, so it borrows `.listing`.
            publish(.listing, for: tier)
            try WizardsperPaths.ensureApplicationSupport()
            try manager.createDirectory(at: WizardsperPaths.models, withIntermediateDirectories: true)
            ...
            let staging = try manager.url(
                ...
                appropriateFor: WizardsperPaths.models, create: true)
            defer {
                do {
        ...
        -> [RemoteFile]
    {
        ...
        for entry in entries {
            ...
            guard isSafeRelativePath(relative) else {
                throw WizardsperError.downloadFailed("the listing contains an unsafe path: \(path)")
            }
    ...
    private static func fetch(
        ...
        guard let url = components.url else {
            throw WizardsperError.downloadFailed("could not build a download URL for \(file.remotePath)")
        }
        ...
        guard let landed = fileSize(at: partial), landed > 0 else {
            throw WizardsperError.downloadFailed("\(file.relativePath) arrived empty")
        }
    ...
    private static func retrying<T>(
        ...
        for attempt in 1...maxAttempts {
            ...
            do {
                ...
                throw WizardsperError.downloadFailed(failure.reason)
            } catch is CancellationError {
                throw CancellationError()
            } catch {
                ...
                guard isTransient(error), attempt < maxAttempts else {
                    ...
                    throw WizardsperError.downloadFailed("\(label) — \(lastReason)")
                }
                Log.model.notice(
    ...
    private static func describe(_ error: any Error) -> String {
        ...
        if let wizardsper = error as? WizardsperError { return wizardsper.errorDescription ?? "\(wizardsper)" }
    ...
    private static func unpack(_ zipURL: URL, into directory: URL, control: ProcessBox) throws {
        ...
        guard FileManager.default.isExecutableFile(atPath: unzip) else {
            throw WizardsperError.downloadFailed("\(unzip) is missing, so the archive cannot be opened")
        }
        guard FileManager.default.fileExists(atPath: zipURL.path) else {
            throw WizardsperError.downloadFailed("\(zipURL.lastPathComponent) does not exist")
        }
        ...
        guard names.isEmpty == false else {
            throw WizardsperError.downloadFailed("\(zipURL.lastPathComponent) is empty")
        }
        for name in names {
