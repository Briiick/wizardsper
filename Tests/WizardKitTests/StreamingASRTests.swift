...
private enum Fixtures {
    static var modelDirectory: URL? {
        let candidates = [
            WizardPaths.modelDirectory(for: .ms560),
    ...
    static var speech: URL? {
        let url = URL(fileURLWithPath: "/tmp/wizard_audio/librispeech/ls_00.wav")
...
struct NemotronConfigTests {
    ...
    private func writeMetadata(_ json: String) throws -> URL {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("wizard-config-\(UUID().uuidString)", isDirectory: true)
    ...
    func rejectsInconsistentFrames() throws {
        ...
        #expect(throws: WizardError.self) { try NemotronConfig(contentsOf: file) }
    }
...
struct StreamingASRTests {
    ...
    func rejectsInvalidFraming() async throws {
        let directory = try #require(Fixtures.modelDirectory)
        let bundle = try await ModelBundle.load(from: directory)
        #expect(throws: WizardError.self) {
            _ = try StreamingASR(
                bundle: bundle,
