...
private func writeTemp(_ contents: String, _ name: String) throws -> URL {
    let url = FileManager.default.temporaryDirectory
        .appendingPathComponent("wizardsper-tests-\(UUID().uuidString)", isDirectory: true)
...
struct TokenizerTests {
    ...
    func rejectsMalformed() throws {
        ...
        #expect(throws: WizardsperError.self) { try NemotronTokenizer(contentsOf: notAnObject) }

        let badKey = try writeTemp(#"{"abc":"x"}"#, "tokenizer.json")
        #expect(throws: WizardsperError.self) { try NemotronTokenizer(contentsOf: badKey) }

        let empty = try writeTemp("{}", "tokenizer.json")
        #expect(throws: WizardsperError.self) { try NemotronTokenizer(contentsOf: empty) }
    }
}
